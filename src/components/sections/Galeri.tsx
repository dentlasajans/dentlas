...
const getOptimizedSrc = (src: string) => {
  return src.replace('q=80', 'q=40').replace('w=1200', 'w=400');
};

const GalleryItem = ({ src, type, onClick }: { src: string, type: 'image' | 'video', onClick: () => void }) => {
  return (
    <motion.div 
      layout
    ...
    >
      ...
      <img src={getOptimizedSrc(src)} alt="Galeri" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" />
              ...
              {selectedMedia.type === 'image' ? (
                ...
                <div className="w-full aspect-video rounded-xl overflow-hidden bg-black/50 shadow-2xl relative flex items-center justify-center group ring-1 ring-white/10">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                      <p className="text-white font-bold bg-black/60 px-4 py-2 rounded-xl backdrop-blur-md translate-y-20">Örnek Video Görüntüsü</p>
                  </div>
                </div>
              )}
            </motion.div>
